...
async def main():
    ...
    tester = Cast.tester()
    await tester.configure_browser(True)
    while i < total:
        ...
        if i == 0:
            # await (await tester.is_on_mz()).log_in_with_user("arcano19").and_pass("9c7ca79c7ca7")
            await page.get_by_role("link", name="Allow all cookies").click()
            await page.get_by_role("textbox", name="Usuario").fill("arcano19")
            await page.get_by_role("textbox", name="Contraseña").fill("9c7ca79c7ca7")
            await page.get_by_role("link", name="Ingresar", exact=True).click()
